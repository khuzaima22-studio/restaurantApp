...
function ManageUsers() {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const res = await fetch('https://restaurantapp-5mka.onrender.com/api/getManagers/ma', {
        headers: {
  ...
  const handleEditSave = async (id) => {
    setLoading(true);
    try {
      const res = await fetch(`https://restaurantapp-5mka.onrender.com/api/getManagers/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
